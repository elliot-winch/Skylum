...
public class AeroSurface : MonoBehaviour
{
    ...
    public FlapTopic FlapAngle { get; } = new();

    public class FlapTopic: Topic<float>
    {
        public override float Value 
        { 
            get => base.Value; 
            set => base.Value = SetFlapAngle(value);
        }

        private float SetFlapAngle(float angle)
        {
            return Mathf.Clamp(angle, -Mathf.Deg2Rad * 50, Mathf.Deg2Rad * 50);
        }
    }
    ...
    public BiVector3 CalculateForces(Vector3 worldAirVelocity, float airDensity, Vector3 relativePosition)
    {
        ...
        float deltaLift = correctedLiftSlope * flapEffectivness * FlapEffectivnessCorrection(FlapAngle.Value) * FlapAngle.Value;
                                          ...
                                          float stallAngleLow)
    {
        ...
        float paddingAngleHigh = Mathf.Deg2Rad * Mathf.Lerp(15, 5, (Mathf.Rad2Deg * FlapAngle.Value + 50) / 100);
        float paddingAngleLow = Mathf.Deg2Rad * Mathf.Lerp(15, 5, (-Mathf.Rad2Deg * FlapAngle.Value + 50) / 100);
                                                 ...
                                                 float stallAngleLow)
    {
        ...
        float normalCoefficient = FrictionAt90Degrees(FlapAngle.Value) * Mathf.Sin(effectiveAngle) *
